...
const versionRows = [
  ...
  ["输出标签数", 19, "对线分路、勾兵、断线、拉野、囤野/堆野、控符、蹲人、开雾、GANK、小规模冲突、团战、肉山团、肉山击杀、高地团、塔下、肉山尝试、魔晶团、资源获得、带盾阵亡", ""],
...
const labelRows = [
  ...
  ["勾兵", "candidate_review", "B", "combat_logs; tower_status_update; dota_model_neutral_siege_creep; derived: lane_zone", "某阵营某一路线上小兵死亡；路线按小兵来源路source_lane判定，优先用ehandle出生/早期轨迹验证，缺失时用死亡坐标兜底；该阵营该路线一塔仍存活；死亡位置在该路线lane_zone外且不在双方一塔保护区内；死亡小兵500范围内存在敌方线上小兵；若敌方英雄获得该小兵死亡经验，则输出勾兵", "通常00:00-10:00", "lane_zone外；一塔保护区12 raw内跳过；附近敌方小兵半径500码；连续小兵死亡默认10秒窗口合并", "获得经验的敌方英雄；多人获得经验则全部列入；结果中附获得经验英雄0-5分钟主对线路", "结果=获得经验英雄获得被勾兵阵营某来源路线小兵死亡经验；可附死亡小兵数量、附近敌方小兵证据和source_lane_method", "单个小兵死亡作为证据；按获得经验阵营+被勾兵阵营+source_lane聚合，连续窗口合并输出；塔下吃兵不输出", "与断线互斥：附近无敌方小兵为断线证据，附近有敌方小兵且敌方英雄获经验则输出勾兵", "需要lane_zone、活体线上小兵位置、经验获得明细和source_lane；缺失个体轨迹时必须在evidence标记xy_fallback", "v1.10草案", ""],
  ["断线", "candidate_review", "B", "combat_logs; tower_status_update; dota_model_neutral_siege_creep; derived: lane_zone", "10分钟前某阵营某一路线上小兵死亡；路线按小兵来源路source_lane判定，优先用ehandle出生/早期轨迹验证，缺失时用死亡坐标兜底；该阵营该路线一塔仍存活；死亡位置在该路线lane_zone外且不在双方一塔保护区内；死亡小兵500范围内无敌方线上小兵；击杀来源可映射为敌方英雄", "通常00:00-10:00", "lane_zone外；一塔保护区12 raw内跳过；附近敌方小兵半径500码；同一英雄同一source_lane 10秒内连续触发>=2或>=3个小兵死亡后输出", "执行断兵的敌方英雄；结果中附执行英雄0-5分钟主对线路", "结果=断兵英雄主对线路、被断兵阵营、小兵来源路线、小兵数量、一塔存活证据；例：中路英雄在下路断下路兵", "单个小兵死亡只作为证据；按击杀英雄+被断兵阵营+source_lane聚合，连续10秒窗口达到阈值才输出事件；塔下吃兵不输出", "与拉野、正常兵线交汇清线互斥；一塔已掉后不判断线", "需要lane_zone、活体线上小兵位置和source_lane；若缺少个体轨迹，必须在evidence标记xy_fallback", "v1.10草案", ""],
  ["拉野", "candidate_review", "B", "combat_logs; dota_model_neutral_siege_creep; player_intervals2", "15分钟前线上小兵与中立单位发生直接伤害/死亡交互；combat直接交互作为锚点，若连续伤害缺失，则用锚点前45秒到后5秒内线兵-同名野怪距离<=550码的状态贴近补证据；若没有combat锚点，则全局扫描状态表中的550码内连续贴近与双方掉血片段；中路排除；路线优先用线兵ehandle全段主路线", "00:00-15:00", "拉野组间隔35秒；位置距离28 raw；状态贴近距离550码（约4.23 raw）；执行者交互窗口为拉野开始前30秒到后10秒；位置兜底800码；附近英雄1500码", "heroes输出推断出的拉野执行者；优先取同阵营英雄/召唤物与该组野怪的交互者，找不到再取800码内最近同阵营英雄；附近双方英雄只写结果/evidence", "结果=某方某路小兵与中立单位连续交战，并保留附近英雄环境证据；evidence写pull_executor method=neutral_interaction或position_nearest；无ehandle时evidence写synthetic状态点", "死亡<2且交互<3则过滤；无combat锚点的状态片段需连续秒数和线兵/野怪掉血门槛", "与断线、普通野区交互需复核", "英雄拉仇恨起点仍需人工核查；召唤物交互按sourcename/targetsourcename归属英雄", "v1.10草案", ""],
  ...
  ["控符", "auto", "A/B", "match_chat_events; player_intervals2", "RUNE_PICKUP/RUNE_BOTTLE/RUNE_DENY；智慧/赏金/水符/护盾/能量符统一为控符；能量符未刷新侧若有英雄也记录控符；value=5赏金神符单独走四赏金符点逻辑：上河道、下河道、天辉赏金server=(590,-4637)/raw≈(133.6,90.4)、夜魇赏金server=(-1000,4443)/raw≈(120.0,166.8)；value=8智慧符单独走两智慧符点逻辑：天辉智慧raw≈(66.0,134.0)/server≈(-7468,610)、夜魇智慧raw≈(191.0,118.0)/server≈(7537,-1285)", "结果事件t-5到t；智慧符t-10到t；未刷新侧spawn_time-5到spawn_time", "符点附近1000码；能量符刷新间隔120秒；罐装后90秒内同英雄同神符拾取去重", "heroes=某符点附近两方英雄；赏金符按四个赏金点附近英雄记录；智慧符按两个智慧符点附近英雄记录", "结果=英雄名+拾取/罐装/反补+上河道/下河道/天辉赏金符点/夜魇赏金符点/天辉智慧符点/夜魇智慧符点+神符；未刷新侧=神符刷新在另一侧；无结果可留空", "每颗神符只记录一次；value=5按四赏金符点区分；value=8按两智慧符点区分；罐装优先于后续拾取", "控符统一标签，不再输出帮控符/控智慧神符", "符点归属和附近英雄可人工复核", "v1.10草案", ""],
  ...
  ["GANK", "candidate_review", "B", "combat_logs; player_intervals2", "发起阶段 N 抓 1：短窗口内同一目标被同一方至少2名英雄命中，且发起时刻目标附近被抓方只有该目标1人；被抓目标承受非远程主战斗伤害>=600或死亡才保留；2000码外贡献者只标远程，不增加参与人数", "发起时刻t0所在战斗窗口", "GANK_OPEN_WINDOW=8秒；t0取第二名不同进攻方英雄第一次命中目标；读取t0±1秒英雄位置；以被抓目标为中心1600码统计附近英雄；gank_victim_damage_min=600；远程贡献阈值2000码", "gank方发起英雄 + 被抓英雄 + 后续支援/反打英雄；远程贡献者写作英雄（远程）", "结果=GANK方、被抓目标、后续支援/反打、击杀比分和死亡列表；evidence写damage_taken_by_hero；1换1仍保留GANK语义", "优先于小规模冲突；后续TP/走入/支援不取消GANK；远程贡献者不计入人数；被抓目标未死亡且承伤<600的2v1消耗不输出为GANK；塔附近信息写入结果/evidence，不自动改成塔下", "高于小规模冲突；默认不被塔下/肉山团等上下文改写", "需要人工校准远程消耗、多人已在场、支援是否已在t0±1秒战场半径内等边界", "v1.10草案", ""],
  ["小规模冲突", "candidate_review", "B", "combat_logs; match_chat_events; player_intervals2", "双方局部伤害/控制/死亡聚类；通常双方各2人以上且总人数<=7，或总人数>=4且有死亡但未达团战规模", "聚类开始-结束；候选先手可提前到控制/大伤害/先手技能时刻", "跨信号间隔12秒；用尾部动态中心合并：最近12秒、最多8条有位置的信号平均位置到新信号距离<=2200码才合并；英雄参与默认1600码", "只保留战斗核心附近或对参与英雄造成伤害/控制的英雄；排除远处未命中核心的独立消耗", "结果=击杀比分；死亡列表；无死亡写未记录英雄击杀/死亡无", "v1.7+吸收旧单点候选；v1.10使用尾部动态中心，允许边打边走/追击累计到同一事件，同时避免跨区域消耗串联", "低于团战，高于纯资源/控符事实", "边界和语义建议录像复核", "v1.10草案", ""],
  ["团战", "candidate_review", "B", "combat_logs; match_chat_events; player_intervals2", "双方多人持续交战；双方总参与通常各4人以上，且直接参与各>=3人，并满足总伤害>=2500、死亡>=2或持续>=15秒之一", "聚类开始-结束；先手技能可提前开始", "团战人数阈值4v4；直接参与3v3；伤害2500；死亡2；持续15秒", "同小规模冲突，按空间/交互排除远端独立交互", "结果=击杀比分；死亡列表；带盾阵亡需标注", "与特殊战斗标签可并存：肉山团/高地团/塔下/魔晶团", "优先级高于小规模冲突；被特殊战斗标签修饰", "远距离同时开战要拆分，不可只按时间合并", "v1.4-v1.9", ""],
  ["肉山团", "candidate_review", "B", "combat_logs; match_chat_events; player_intervals2", "肉山仍存活，且战斗围绕肉山坑/肉山伤害/肉山击杀上下文展开；团战/小规模冲突中心在肉山上路坑或下路坑2000码内时，记录为对应坑位战斗", "战斗窗口；肉山伤害前后30秒；肉山击杀前45秒到后75秒", "肉山上路坑server=(-3084,2296)，raw≈(102.6,148.3)；肉山下路坑server=(2842,-2743)，raw≈(152.1,105.9)；坑位半径2000码；肉山死亡后480秒内视为未刷新", "参与战斗双方英雄", "结果追加目标：肉山相关；坑位空间命中时补充“肉山上路坑战斗/肉山下路坑战斗”；肉山击杀结果由肉山击杀标签独立记录", "坑位空间规则只修饰团战/小规模冲突，不改写GANK/单杀；肉山死亡后的追击不再因靠近肉山坑误标肉山团", "可与团战/小规模冲突并存", "需区分肉山尝试、肉山击杀、肉山后追击", "v1.4-v1.10草案", ""],
  ...
  ["塔下", "candidate_review", "B", "tower_status_update; combat_logs", "一方在防御塔附近约1000码内防守时发生的战斗", "战斗开始前5秒到结束后5秒内塔上下文", "防御塔1000码；塔hp>0", "参与战斗双方英雄", "结果继承战斗结果并附塔上下文", "作为附加标签追加，可与小规模冲突/团战并存", "特殊上下文标签", "塔已掉或追击阶段不应误标", "v1.4-v1.9", ""],
...
const parameterRows = [
  ...
  ["lane_zone_tower_protect_radius", "12", "raw坐标", "对线区", "该路线任一存活一塔附近视为正常塔下吃兵区，不进入断线/勾兵候选", "v1.10草案", "是", ""],
  ["lane_creep_source_early_seconds", "8", "秒", "对线区", "有ehandle时用小兵最早出现后8秒内轨迹判断source_lane", "v1.10草案", "是", ""],
  ...
  ["pull_status_proximity_game", "550", "码", "拉野", "线兵与中立野怪状态贴近的最大距离；有combat锚点时做锚点附近扫描，无combat锚点时做全局状态扫描；实现时换算为raw约4.23", "v1.10草案", "是", ""],
  ["pull_status_anchor_pre_window", "45", "秒", "拉野", "combat锚点前回溯状态贴近证据的最大窗口", "v1.10草案", "是", ""],
  ["pull_status_anchor_post_window", "5", "秒", "拉野", "combat锚点后补充状态贴近证据的最大窗口", "v1.10草案", "是", ""],
  ...
  ["radiant_bounty_rune_spot", "server=(590,-4637); raw≈(133.6,90.4)", "坐标", "控符", "天辉赏金符点；raw由当前homography从server坐标换算", "v1.10草案", "是", ""],
  ["dire_bounty_rune_spot", "server=(-1000,4443); raw≈(120.0,166.8)", "坐标", "控符", "夜魇赏金符点；raw由当前homography从server坐标换算", "v1.10草案", "是", ""],
  ["radiant_wisdom_rune_spot", "raw≈(66.0,134.0); server≈(-7468,610)", "坐标", "控符", "天辉智慧符点；match_chat_events不直接给x/y，用拾取英雄同秒位置校准", "v1.10草案", "是", ""],
  ["dire_wisdom_rune_spot", "raw≈(191.0,118.0); server≈(7537,-1285)", "坐标", "控符", "夜魇智慧符点；match_chat_events不直接给x/y，用拾取英雄同秒位置校准", "v1.10草案", "是", ""],
  ...
  ["combat_cluster_merge_distance_game", "2200", "码", "小规模冲突/团战", "相邻战斗信号合并最大中心距离；超过则拆成独立战斗", "v1.10草案", "是", ""],
  ["combat_cluster_tail_seconds", "12", "秒", "小规模冲突/团战", "尾部动态中心时间窗口；只取最近该秒数内的信号位置参与合并中心计算", "v1.10草案", "是", ""],
  ["combat_cluster_tail_signal_count", "8", "条", "小规模冲突/团战", "尾部动态中心最多使用的最近有位置信号数", "v1.10草案", "是", ""],
  ...
  ["remote_contribution_distance", "2000", "?", "GANK/?????/??", "????2000???????????/BUFF?????????????????????????????????", "v1.10??", "?", ""],
  ...
  ["gank_open_window", "8", "秒", "GANK", "同一目标在短窗口内被同一方至少2名不同英雄命中，才进入GANK候选", "v1.10草案", "是", ""],
  ["gank_position_sample_window", "±1", "秒", "GANK", "读取发起时刻t0前后1秒的英雄位置；同一英雄取距离t0最近的位置", "v1.10草案", "是", ""],
  ["gank_position_radius", "1600", "码", "GANK", "以被抓目标位置为中心统计发起阶段附近英雄", "v1.10草案", "是", ""],
  ["gank_min_attacker_nearby_heroes", "2", "人", "GANK", "发起时刻战场半径内GANK方至少2名英雄，且至少2名英雄直接命中同一目标", "v1.10草案", "是", ""],
  ["gank_max_victim_nearby_heroes", "1", "人", "GANK", "发起时刻战场半径内被抓方只有被抓目标1名英雄；后续TP/走入只影响结果，不取消GANK", "v1.10草案", "是", ""],
  ["gank_victim_damage_min", "600", "伤害", "GANK", "N抓1成立后，被抓目标承受非远程主战斗伤害达到该阈值才保留；死亡可替代", "v1.10草案", "是", ""],
  ...
  ["roshan_upper_pit_server", "(-3084,2296)", "server坐标", "肉山团", "肉山上路坑中心点；换算raw约(102.6,148.3)", "v1.10草案", "是", ""],
  ["roshan_lower_pit_server", "(2842,-2743)", "server坐标", "肉山团", "肉山下路坑中心点；换算raw约(152.1,105.9)", "v1.10草案", "是", ""],
  ...
  ["tower_context_radius", "1000", "码", "塔下", "防御塔上下文范围", "v1.4", "是", ""],
  ["tower_context_time_pad", "5", "秒", "塔下", "战斗窗口前后塔状态检测", "v1.4", "是", ""],
...
const dataSourceRows = [
  ...
  ["dota_model_neutral_siege_creep", "time, log_index, unit, team, ehandle, x, y, hp, max_hp, lifeState", "线上小兵/中立野怪状态与轨迹", "对线区; 勾兵; 断线; 拉野", "按ehandle追踪小兵路线；判断死亡小兵500范围内是否有敌方线上小兵；拉野可在combat锚点附近或无锚点全局扫描中用线兵-野怪状态贴近补证据，无ehandle时用synthetic状态点兜底", "默认只需抽取-5到900秒"],
  ...
  ["tower_status_update", "time, type, team_num, hp, max_hp, x, y", "防御塔状态", "塔下", "判断战斗是否发生在存活防御塔附近", ""],
...
const flowRows = [
  ...
  ["07", "战斗标签判定", "fight_records", "先按发起时刻位置判断GANK：同一目标短窗口被同一方至少2人命中，读取t0±1秒位置，若进攻方附近>=2且被抓方附近=1则输出GANK；否则按参与人数、直接参与人数、伤害、死亡、持续时间输出小规模冲突或团战", "战斗类事件", "边界事件仍需人工复核", "旧抓人候选不再作为最终标签，新增GANK承接发起阶段N抓1语义", ""],
  ["08", "特殊战斗上下文", "fight_records; objective_events; tower_status_update", "给战斗追加肉山团、高地团、塔下、魔晶团", "多标签战斗事件", "资源死亡后的追击误标资源团", "用资源存活约束和死亡后抑制窗口", ""],
...
const mergeRows = [
  ...
  ["M06", "旧单点候选归类", "历史抓人候选", "符合发起阶段位置N抓1则GANK；与已有团战窗口重叠则团战；与已有小规模冲突重叠则小规模冲突；否则总人数>=8且双方>=3为团战，其余为小规模冲突", "输出新标签，不输出抓人", "删除抓人后仍保留战斗语义", "v1.6/v1.7/v1.10", "是", ""],
