...
const labelRows = [
  ...
  ["拉野", "candidate_review", "B", "combat_logs; dota_model_neutral_siege_creep; player_intervals2", "15分钟前线上小兵与中立单位发生直接伤害/死亡交互；combat直接交互作为锚点，若连续伤害缺失，则用锚点前8秒到后2秒内线兵-同名野怪距离<=9 raw的状态贴近补证据；中路排除", "00:00-15:00", "拉野组间隔35秒；位置距离28 raw；状态贴近距离9 raw；附近英雄1500码", "附近双方/己方英雄，去重输出", "结果=某方某路小兵与中立单位连续交战；无ehandle时evidence写synthetic状态点", "死亡<2且交互<3则过滤；只有状态贴近、无combat锚点不输出", "与断线、普通野区交互需复核", "英雄拉仇恨起点仍需人工核查", "v1.10草案", ""],
  ...
  ["GANK", "candidate_review", "B", "combat_logs; player_intervals2", "发起阶段 N 抓 1：短窗口内同一目标被同一方至少2名英雄命中，且发起时刻目标附近被抓方只有该目标1人", "发起时刻t0所在战斗窗口", "GANK_OPEN_WINDOW=8秒；t0取第二名不同进攻方英雄第一次命中目标；读取t0±1秒英雄位置；以被抓目标为中心1600码统计附近英雄", "gank方发起英雄 + 被抓英雄 + 后续支援/反打英雄", "结果=GANK方、被抓目标、后续支援/反打、击杀比分和死亡列表；1换1仍保留GANK语义", "优先于小规模冲突；后续TP/走入/支援不取消GANK；塔附近信息写入结果/evidence，不自动改成守塔团", "高于小规模冲突；默认不被守塔团/肉山团等上下文改写", "需要人工校准远程消耗、多人已在场、支援是否已在t0±1秒战场半径内等边界", "v1.10草案", ""],
...
const parameterRows = [
  ...
  ["pull_status_proximity_raw", "9", "raw坐标", "拉野", "combat锚点附近线兵与同名中立野怪状态贴近的最大距离", "v1.10草案", "是", ""],
...
const dataSourceRows = [
  ...
  ["dota_model_neutral_siege_creep", "time, log_index, unit, team, ehandle, x, y, hp, max_hp, lifeState", "线上小兵/中立野怪状态与轨迹", "对线区; 勾兵; 断线; 拉野", "按ehandle追踪小兵路线；判断死亡小兵500范围内是否有敌方线上小兵；拉野在combat锚点附近用线兵-野怪状态贴近补证据，无ehandle时用synthetic状态点兜底", "默认只需抽取-5到900秒"],
