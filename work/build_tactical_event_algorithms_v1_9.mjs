...
const versionRows = [
  ...
  ["输出标签数", 21, "对线分路、出门抢远程兵、出门抢近战兵、勾兵、断线、拉野、囤野/堆野、控符、蹲人、开雾、GANK、小规模冲突、团战、肉山团、肉山击杀、高地团、塔下、肉山尝试、魔晶团、资源获得、带盾阵亡", ""],
...
const labelRows = [
  ...
  ["出门抢远程兵", "auto", "A/B", "combat_logs; dota_model_neutral_siege_creep", "0-31秒内上路/下路远程兵死亡；判断该兵来源路线、补刀/金钱获得者、经验分享者；中路不识别", "00:00-00:31", "死亡日志前后1秒相邻日志块；排除开局发钱gold_reason=17和负数金钱；路线优先ehandle早期轨迹，缺失用死亡点坐标；source_lane=mid直接跳过", "heroes输出金钱获得者、经验获得者、击杀来源英雄", "结果=某方某路远程兵死亡；补到/获得金钱英雄；金钱分配；经验分配", "每个死亡远程兵一行，不聚合", "早期边路资源争夺事实类；低于战斗类", "如果金钱日志缺失，用死亡击杀来源兜底；需人工核查反补/召唤物击杀边界", "v1.11草案", ""],
  ["出门抢近战兵", "auto", "A/B", "combat_logs; dota_model_neutral_siege_creep", "0-31秒内上路/下路近战兵死亡；判断该兵来源路线、补刀/金钱获得者、经验分享者；中路不识别", "00:00-00:31", "死亡日志前后1秒相邻日志块；排除开局发钱gold_reason=17和负数金钱；路线优先ehandle早期轨迹，缺失用死亡点坐标；source_lane=mid直接跳过", "heroes输出金钱获得者、经验获得者、击杀来源英雄", "结果=某方某路近战兵死亡；补到/获得金钱英雄；金钱分配；经验分配", "每个死亡近战兵一行，不聚合", "早期边路资源争夺事实类；低于战斗类", "如果金钱日志缺失，用死亡击杀来源兜底；需人工核查反补/召唤物击杀边界", "v1.11草案", ""],
  ...
  ["拉野", "candidate_review", "B", "combat_logs; dota_model_neutral_siege_creep; player_intervals2", "15分钟前线上小兵与中立单位发生直接伤害/死亡交互；combat直接交互作为锚点，若连续伤害缺失，则用锚点前45秒到后5秒内线兵-同名野怪距离<=550码的状态贴近补证据；若没有combat锚点，则全局扫描状态表中的550码内连续贴近与双方掉血片段；中路排除；路线优先用线兵ehandle全段主路线；先确认野怪被拉动并勾走线兵仇恨，最后线兵与野怪连续交战/贴近停滞", "00:00-15:00", "拉野组间隔35秒；位置距离28 raw；状态贴近距离550码（约4.23 raw）；执行者仇恨交互窗口为拉野开始前12秒到后10秒；同一波8秒内多候选只保留最早仇恨触发者；附近英雄1500码", "heroes输出推断出的拉野执行者；先判定拉野链条成立，再从野怪移动最早阶段反推谁触发该组野怪仇恨；当前实现以英雄主动打到该组野怪或该组野怪最早打到英雄的第一条日志为强证据，不再用位置兜底归因；附近双方英雄只写结果/evidence", "结果=某方某路小兵与中立单位连续交战，并保留附近英雄环境证据；evidence写pull_executor method=neutral_interaction；无ehandle时evidence写synthetic状态点", "死亡<2且交互<3则过滤；无combat锚点的状态片段需连续秒数和线兵/野怪掉血门槛；无英雄-野怪仇恨交互证据不输出执行者", "与断线、普通野区交互需复核", "后续需补全英雄走近但无伤害的移动归因；召唤物交互按sourcename/targetsourcename归属英雄", "v1.10草案", ""],
  ...
  ["小规模冲突", "candidate_review", "B", "combat_logs; match_chat_events; player_intervals2", "双方局部伤害/控制/死亡聚类；通常双方各2人以上且总人数<=7，或总人数>=4且有死亡但未达团战规模；前10分钟使用对线期分段口径，单条伤害阈值30、跨信号间隔7秒；短事件<=12秒保留单个英雄承伤>400或总伤害>900；长事件>12秒改看滑动8秒爆发窗口，8秒内总伤害>=700或单英雄承伤>=360或有死亡才保留；10分钟后维持旧口径，单条伤害阈值45、跨信号间隔12秒，总伤害>=550或有英雄死亡即可保留", "聚类开始-结束；候选先手可提前到控制/大伤害/先手技能时刻", "0<=time<600：跨信号间隔7秒，尾部动态中心最近7秒；duration<=12秒看整段单英雄承伤>400或总伤害>900；duration>12秒看滑动8秒窗口，总伤害>=700或单英雄承伤>=360或有死亡；time>=600：跨信号间隔12秒，尾部动态中心最近12秒；两个阶段均最多8条有位置信号，中心到新信号距离<=2200码才合并；英雄参与默认1600码", "只保留战斗核心附近或对参与英雄造成伤害/控制的英雄；排除远处未命中核心的独立消耗", "结果=击杀比分；死亡列表；无死亡写未记录英雄击杀/死亡无", "v1.7+吸收旧单点候选；v1.10使用尾部动态中心，允许边打边走/追击累计到同一事件，同时避免跨区域消耗串联；前10分钟长窗口必须有明确爆发，避免对线换血被过长串联或低强度死亡单独带出", "低于团战，高于纯资源/控符事实", "边界和语义建议录像复核", "v1.10草案", ""],
  ...
  ["肉山团", "candidate_review", "B", "combat_logs; match_chat_events; player_intervals2", "15分钟后，团战/小规模冲突中读取开战后的前3个秒点(start,start+1,start+2)，对所有参与英雄本体坐标求平均；该平均坐标在肉山坑1000码内时，记录为对应坑位战斗；肉山击杀/掉落只作为资源事实，不作为肉山团充分条件", "15:00后；战斗开始后前3秒", "肉山上路坑server=(-2924.964844,1522.657715)，raw≈(104.0,141.7)；肉山下路坑server=(2867.646973,-3401.379395)，raw≈(152.3,100.5)；坑位半径1000码；15分钟前不计算肉山团", "参与战斗双方英雄", "结果追加目标：肉山相关；坑位空间命中时补充“肉山上路坑战斗/肉山下路坑战斗”；肉山击杀结果由肉山击杀标签独立记录", "不再用任意一条伤害/控制/死亡交互点靠近肉山坑作为充分条件；坑位空间规则只修饰15分钟后的团战/小规模冲突，不改写GANK/单杀；肉山死亡后的追击不再因靠近肉山坑误标肉山团", "可与团战/小规模冲突并存", "需区分肉山尝试、肉山击杀、肉山后追击；15分钟前即使靠近肉山坑也不标肉山团", "v1.11草案", ""],
...
const parameterRows = [
  ["opening_creep_contest_max_time", "31", "seconds", "出门抢远程兵/近战兵", "只统计0-31秒内死亡的上路/下路远程兵/近战兵，中路不识别", "v1.11草案", "是", ""],
  ["opening_creep_reward_window", "1", "seconds", "出门抢远程兵/近战兵", "读取小兵死亡日志前后1秒相邻日志块中的金钱和经验", "v1.11草案", "是", ""],
  ["opening_creep_gold_exclude_reason", "17", "gold_reason", "出门抢远程兵/近战兵", "排除开局发钱；负数金钱也不计入金钱获得者", "v1.11草案", "是", ""],
  ...
  ["combat_damage_min_event_pre10", "30", "伤害", "小规模冲突/团战", "0<=time<600 时进入战斗检测的单条伤害最小值", "v1.10草案", "是", ""],
  ["combat_damage_min_event_post10", "45", "伤害", "小规模冲突/团战", "time>=600 时进入战斗检测的单条伤害最小值；10分钟后计算方式不变", "v1.4", "是", ""],
  ...
  ["combat_cluster_merge_gap_pre10", "7", "秒", "小规模冲突/团战", "0<=time<600 时相邻战斗信号合并最大时间间隔", "v1.10草案", "是", ""],
  ["combat_cluster_merge_gap_post10", "12", "秒", "小规模冲突/团战", "time>=600 时相邻战斗信号合并最大时间间隔；10分钟后计算方式不变", "v1.4", "是", ""],
  ["combat_cluster_merge_distance_game", "2200", "码", "小规模冲突/团战", "相邻战斗信号合并最大中心距离；超过则拆成独立战斗", "v1.10草案", "是", ""],
  ["combat_cluster_tail_seconds_pre10", "7", "秒", "小规模冲突/团战", "0<=time<600 时尾部动态中心时间窗口；只取最近该秒数内的信号位置参与合并中心计算", "v1.10草案", "是", ""],
  ["combat_cluster_tail_seconds_post10", "12", "秒", "小规模冲突/团战", "time>=600 时尾部动态中心时间窗口；10分钟后计算方式不变", "v1.10草案", "是", ""],
  ["combat_cluster_tail_signal_count", "8", "条", "小规模冲突/团战", "尾部动态中心最多使用的最近有位置信号数", "v1.10草案", "是", ""],
  ["skirmish_pre10_single_hero_damage_threshold", "400", "伤害", "小规模冲突", "0<=time<600 小规模冲突保留条件：任一英雄承伤必须 >400；也可由总伤害>900替代", "v1.10草案", "是", ""],
  ["skirmish_pre10_total_damage_threshold", "900", "伤害", "小规模冲突", "0<=time<600 小规模冲突保留条件：总伤害必须 >900；也可由单英雄承伤>400替代", "v1.10草案", "是", ""],
  ["skirmish_pre10_short_duration_seconds", "12", "秒", "小规模冲突", "0<=time<600 且 duration<=12秒时使用整段伤害保留条件", "v1.10草案", "是", ""],
  ["skirmish_pre10_burst_window_seconds", "8", "秒", "小规模冲突", "0<=time<600 且 duration>12秒时使用滑动爆发窗口", "v1.10草案", "是", ""],
  ["skirmish_pre10_burst_total_damage_threshold", "700", "伤害", "小规模冲突", "长事件滑动8秒窗口内总伤害 >=700 才保留；也可由单英雄承伤>=360或死亡替代", "v1.10草案", "是", ""],
  ["skirmish_pre10_burst_single_hero_damage_threshold", "360", "伤害", "小规模冲突", "长事件滑动8秒窗口内任一英雄承伤 >=360 才保留；也可由总伤害>=700或死亡替代", "v1.10草案", "是", ""],
  ...
  ["roshan_upper_pit_server", "(-2924.964844,1522.657715)", "server坐标", "肉山团", "肉山上路坑中心点；换算raw约(104.0,141.7)", "v1.10草案", "是", ""],
  ["roshan_lower_pit_server", "(2867.646973,-3401.379395)", "server坐标", "肉山团", "肉山下路坑中心点；换算raw约(152.3,100.5)", "v1.10草案", "是", ""],
  ["roshan_context_min_time", "900", "秒", "肉山团", "15分钟后靠近肉山坑才可触发肉山上下文；15分钟前不计算肉山团", "v1.11草案", "是", ""],
...
const flowRows = [
  ...
  ["16", "合并后重判主标签", "合并后的fight_records", "相邻合并、追击合并或续团合并后，用合并后的双方人数、直接参与人数、总伤害、死亡数和持续时间重新判定主战斗标签；满足团战条件则从小规模冲突升级为团战", "更新后的战斗标签", "第一段小规模冲突合并后达到团战规模但未升级", "8842468101 21:36-22:14 应由小规模冲突升级为团战", "v1.10草案"],
  ["17", "远程交互不作为续团共享英雄", "interaction_positions; player_intervals2", "续团/追击合并取共享交互英雄时，若施加者与目标英雄本体距离>2000码，则远程施加者不进入共享英雄集合；该交互只保留被命中目标作为现场交互英雄", "更干净的续团合并结果", "先知大招等全图远程伤害把两个空间独立战斗误合并", "8842468101 25:28-25:46 拆为两条GANK", "v1.10草案"],
];
...
const mergeRows = [
  ...
  ["M14", "合并后重判主标签", "战斗类标签", "战斗候选完成相邻合并、追击合并或续团合并后，重新按合并后的规模与强度调用主分类规则", "小规模冲突可升级为团战，GANK追击可被父战斗吸收", "修复合并后仍沿用第一段小规模冲突标签的问题", "v1.10草案", "是", ""],
  ["M15", "远程共享英雄排除", "战斗类标签", "续团/追击合并时，距离>2000码的远程施加者不作为共享交互英雄；全图/超远距离技能只作为远程贡献", "避免跨区域战斗被远程技能粘连", "修复先知大招把25:28潮汐战斗和25:33树精死亡合并的问题", "v1.10草案", "是", ""],
];
