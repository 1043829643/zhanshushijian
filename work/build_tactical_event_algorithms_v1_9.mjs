...
const versionRows = [
  ...
  ["输出标签数", 18, "对线分路、勾兵、断线、拉野、囤野/堆野、控符、蹲人、开雾、小规模冲突、团战、肉山团、肉山击杀、高地团、守塔团、肉山尝试、魔晶团、资源获得、带盾阵亡", ""],
  ["已删除标签", "抓人", "不作为最终输出标签；历史候选会重映射/合并为小规模冲突或团战", ""],
...
const dataSourceRows = [
  ...
  ["derived: fight_records", "start, end, center, heroes_by_team, damage_by_team, death_events", "战斗聚类结果", "小规模冲突; 团战; 特殊战斗", "供后处理、标签判定、结果字段生成", ""],
...
const flowRows = [
  ...
  ["07", "战斗标签判定", "fight_records", "按参与人数、直接参与人数、伤害、死亡、持续时间输出小规模冲突或团战", "战斗类事件", "边界事件仍需人工复核", "旧抓人候选不再作为最终标签", ""],
...
const mergeRows = [
  ...
  ["M04", "旧抓人标签删除", "历史抓人候选", "labels含抓人", "删除抓人标签；按规则改为小规模冲突/团战或合并入父事件", "v1.5后最终定义不再有抓人字段", "v1.6", "是", ""],
  ["M05", "旧单点死亡结果补全", "历史抓人候选", "旧单点候选记录了被抓英雄且造成死亡", "结果写击杀比分和死亡英雄，如击杀结果：天辉1-0夜魇；死亡：05:34祈求者", "修复死亡未记录结果", "v1.6", "是", ""],
  ["M06", "旧单点候选归类", "历史抓人候选", "与已有团战窗口重叠则团战；与已有小规模冲突重叠则小规模冲突；否则总人数>=8且双方>=3为团战，其余为小规模冲突", "输出新标签，不输出抓人", "删除抓人后仍保留战斗语义", "v1.6/v1.7", "是", ""],
...
const outputRows = [
  ...
  ["labels", "事件标签", "是", "多个标签用 / 分隔；不得包含抓人", "文本", "来自定义清单18标签", ""],
