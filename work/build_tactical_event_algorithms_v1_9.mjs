...
const labelRows = [
  ...
  ["GANK", "candidate_review", "B", "combat_logs; player_intervals2", "战斗聚类中一方N抓1：一方直接参与英雄>=2，另一方参与英雄<=1", "聚类开始-结束", "继承战斗聚类时间/空间阈值；直接参与=造成伤害或施加控制", "gank方英雄 + 被抓英雄", "结果=击杀比分；死亡列表；无死亡写未记录英雄击杀/死亡无", "优先于小规模冲突判定；后续可被守塔团/高地团/肉山团等特殊上下文修饰", "低于团战特殊上下文，高于小规模冲突", "需要人工校准N抓1但未实质开战、远程消耗等边界", "v1.10草案", ""],
...
const parameterRows = [
  ...
  ["gank_min_direct_attackers", "2", "人", "GANK", "GANK方至少有2名直接参与英雄，直接参与指造成伤害或施加控制", "v1.10草案", "是", ""],
...
const flowRows = [
  ...
  ["07", "战斗标签判定", "fight_records", "先判断GANK：一方直接参与>=2且另一方参与<=1；否则按参与人数、直接参与人数、伤害、死亡、持续时间输出小规模冲突或团战", "战斗类事件", "边界事件仍需人工复核", "旧抓人候选不再作为最终标签，新增GANK承接N抓1语义", ""],
