...
- `GANK` 优先级高于 `小规模冲突`；塔附近信息只写入结果或 evidence，不把 GANK 改成 `塔下`。
...
- 基于已有战斗聚类结果追加上下文标签，不单独生成脱离战斗的塔下。
- `GANK` 默认不升级为塔下。
- 若 `小规模冲突` 或 `团战` 的战斗中心距离一座存活防御塔 `<= 1000` 码，追加 `塔下`。
- 塔存活状态来自 `tower_status_update`：取战斗开始附近最近塔状态，`hp > 0` 视为存活。
- 输出标签格式为 `塔下 / 小规模冲突` 或 `塔下 / 团战`，结果字段追加防守方和塔血量上下文。
- 当前实现不因单方推塔、无双方英雄交战而生成塔下。

当前战斗聚类仍是保守草案：它优先保留可解释证据，已接入 `肉山团` 和 `塔下` 上下文，但对线消耗过滤、其他特殊战斗上下文、相邻战斗合并和人工复核规则仍需继续完善。
...
- 特殊上下文：把 `tower_status_update`、`other_unit_sync`、`hero_roshan_miniboss_vtord` 接入高地团、塔下、肉山团、魔晶团。
